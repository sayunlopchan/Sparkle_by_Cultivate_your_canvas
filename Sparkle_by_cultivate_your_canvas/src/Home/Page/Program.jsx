  ...
  return (
    <div>
      ...
      <div className='space-y-5'>
        ...
        {/* Banner Image */}
        <div
          className="w-90vw h-[260px] md:h-[540px] bg-cover bg-center m-10 border-t-8 border-t-red-500 border-r-8 border-r-green-500 border-b-8 border-b-blue-500 border-l-8 border-l-yellow-500 rounded-3xl"
        ...
        {programData.map((program, index) => (
          <div
            key={index}
            className={`flex ${index % 2 === 0 ? 'max-lg:flex-col' : 'flex-row-reverse max-lg:flex-col'} gap-6 md:gap-24 p-10 items-center`}
          >
            <div className='lg:w-[40%]'>
              <AnimatedComponent>
                <img src={program.image} alt={program.title} className={`animate-animate shadow-xl max-lg:h-[300px] max-sm:h-full max-md:hidden`} />
              </AnimatedComponent>
            </div>
            <div className='lg:w-[60%]'>
              <AnimatedComponent2>
                <div className='flex justify-center items-start flex-col'>
                  ...
                  <h1 className='mx-5 font-bold text-[30px]'>{program.title}</h1>
                </div>
                <p className='p-text' dangerouslySetInnerHTML={{ __html: program.para }} />
              </AnimatedComponent2>
            </div>
          </div>
        ))}
